...
# pd.set_option('display.max_rows', None) #全部打印
# symbol = 'SOL-USDT_5m'
# symbol = 'BTC-USDT_5m'
# symbol = 'AVAX-USDT_5m'
symbol = 'ETH-USDT_5m'
# =====导入数据
df = pd.read_hdf('/Users/caoliang/PycharmProjects/xbx_part_3/data/%s_pos.h5' % symbol, key='df')
# 选取数据：币种上线10天之后的日期
...
df['start_time'].fillna(method='ffill', inplace=True)# 开仓时间
...
face_value = 0.01  #每张合约的面值， btc是0.01，不同的币种要进行不同的替换
...
df.to_hdf('/Users/caoliang/PycharmProjects/xbx_part_3/data/%s_equity_curve.h5' % symbol, key='df', mode='w')
print(df)
exit()
