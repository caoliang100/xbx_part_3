...
# =====简单布林策略
# 策略
def signal_simple_bolling(df, para=[200, 2, 0.05]):
    """
    :param df:
    :param para: n, m, diff
    :return:
    ...
    # 布林线上轨：n天收盘价的移动平均线 - m * n天收盘价的标准差
    # 当收盘价由下向上穿过上轨的时候,并且当前收盘价距离中轨小于diff，做多；然后由上向下穿过中轨的时候，平仓。
    ...
    condition2 = (df['close'] - df['median']) / df['median'] < diff  # 上轨-均值 < diff
    df.loc[condition1 & condition2, 'signal_long'] = 1  # 将产生做多信号的那根K线的signal设置为1，1代表做多
    df.loc[condition1 & condition2, 'diff'] = (df['close'] - df['median']) / df[
        'median']  # 将产生做多信号的那根K线的signal设置为1，1代表做多
    ...
    condition2 = (df['close'] - df['median']) / df['median'] > -diff  # 下轨-均值 >-diff
    df.loc[condition1 & condition2, 'signal_short'] = -1  # 将产生做空信号的那根K线的signal设置为-1，-1代表做空
    df.loc[condition1 & condition2, 'diff'] = (df['close'] - df['median']) / df['median']  # 将产生平仓信号当天的signal设置为0，0代表平仓
    pd.set_option('display.max_rows', 1000)  # 最多显示数据的行数
    # print(df[df['signal'].notnull()])
    # print(df[df['signal_short']==-1])
    # exit()
...
def signal_simple_bolling_para_list(m_list=range(10, 1000, 10), n_list=[i / 10 for i in list(np.arange(5, 50, 1))],
                                    diff_list=[i / 1000 for i in list(np.arange(5, 51, 1))]):
    """
    产生布林 策略的参数范围
    :param m_list:
    :param n_list:
    :param diff_list: 距离均值多少开仓
    :return:
    """
    para_list = []
...
print(len(signal_simple_bolling_para_list()))
# =====简单海龟策略
# 策略
